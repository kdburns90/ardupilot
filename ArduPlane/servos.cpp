...
void Plane::servos_twin_engine_mix(void)
{
    float throttle = SRV_Channels::get_output_scaled(SRV_Channel::k_throttle);
    float rud_gain = float(plane.g2.rudd_dt_gain) / 100;
    float left_elevon = rud_gain * SRV_Channels::get_output_scaled(SRV_Channel::k_vtail_right) / float(SERVO_MAX);
    float right_elevon = rud_gain * SRV_Channels::get_output_scaled(SRV_Channel::k_vtail_left) / float(SERVO_MAX);
